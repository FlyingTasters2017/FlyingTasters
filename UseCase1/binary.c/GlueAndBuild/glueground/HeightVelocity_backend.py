...
def setMsgQ():
    global tmId
    tmId = PythonController.i_HeightVelocity


def setUDP():
    global tmId
    from _InterfaceEnum import i_ground_PI_HeightVelocity
    tmId = i_ground_PI_HeightVelocity


def HeightVelocity(tm_ptr, size):
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.MyReal_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.MyReal_REQUIRED_BYTES_FOR_ENCODING)
...
def expect(Q, VNvalue, ignoreOther=False, timeout=None):
    ...
    while True:
        ...
        if msgId == tmId:
            #expectedValue = '{ HeightVelocity ' + VNvalue + ' }'
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received HeightVelocity with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected HeightVelocity (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print 'Received other (%s), but still waiting for HeightVelocity' % str(msgId)
            Q.task_done()


def fromPysideToASN1(_):
    print('DEPRECATED call to "fromPysideToASN1"')
    # Create a native ASN.1 variable of type MyReal
    # HeightVelocity = ASN1.MyReal()
    return None
