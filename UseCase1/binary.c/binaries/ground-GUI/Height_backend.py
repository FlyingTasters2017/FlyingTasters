...
def setMsgQ():
    global tmId
    tmId = PythonController.i_Height


def setUDP():
    global tmId
    from _InterfaceEnum import i_ground_PI_Height
    tmId = i_ground_PI_Height


def Height(tm_ptr, size):
    ...
    while True:
        ...
        if msgId == tmId:
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received Height with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected Height (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print 'Received other (%s), but still waiting for Height' % str(msgId)
            Q.task_done()
