            ...
            while (trId /= -1) loop
                case trId is
                    when 0 =>
                        ...
                        Put("SDL Startup");
                        New_Line;
                        -- NEXT_STATE Wait (14,18) at 492, 243
                        trId := -1;
                        ctxt.state := Wait;
                        goto next_transition;
                    when 1 =>
                        -- readStabilizerSendThrust(droneData,sensorData) (20,17)
                        RIÜreadStabilizerSendThrust(ctxt.droneData'Access, ctxt.sensorData'Access);
                        -- displaySensor(sensorData) (22,17)
                        RIÜdisplaySensor(ctxt.sensorData'Access);
                        -- NEXT_STATE Running (24,22) at 805, 369
                        trId := -1;
                        ctxt.state := Running;
                        goto next_transition;
                    when 2 =>
                        -- NEXT_STATE Wait (28,22) at 1048, 259
                        trId := -1;
                        ctxt.state := Wait;
                        goto next_transition;
                    when 3 =>
                        -- NEXT_STATE Wait (38,22) at 421, 353
                        trId := -1;
                        ctxt.state := Wait;
                        goto next_transition;
                    when 4 =>
                        -- NEXT_STATE Running (42,22) at 537, 353
                        trId := -1;
                        ctxt.state := Running;
                        goto next_transition;
                    when CS_Only =>
