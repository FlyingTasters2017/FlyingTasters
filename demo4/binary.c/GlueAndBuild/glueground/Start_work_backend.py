...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_Start_work
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_ground_RI_Start_work
    tcId = i_ground_RI_Start_work
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global Start_work_via_shared_lib
    shared_lib = True
    Start_work_via_shared_lib = dll.ground_Start_work
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_Start_work(tc)
            except IOError:
                raise
    elif udp:
