...
def setMsgQ():
    global tmId
    tmId = PythonController.i_Thrust_Update


def setUDP():
    global tmId
    from _InterfaceEnum import i_ground_PI_Thrust_Update
    tmId = i_ground_PI_Thrust_Update


def Thrust_Update(tm_ptr, size):
...
def expect(Q, VNvalue, ignoreOther=False, timeout=None):
    ...
    while True:
        ...
        if msgId == tmId:
            #expectedValue = '{ Thrust-Update ' + VNvalue + ' }'
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received Thrust-Update with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected Thrust-Update (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print 'Received other (%s), but still waiting for Thrust-Update' % str(msgId)
            Q.task_done()
