...
process_package__taste_protected_object gcs_protected;
...
/*!
 * Store the period time for task vt_gcs_sensordata
 */
  __po_hi_milliseconds (&(period), 1);
 ...
 * Making Sporadic task
 */
  __po_hi_create_sporadic_task (x86_partition_vt_gcs_sensordata_k, &(period), 1, 50000, 0, vt_gcs_sensordata_job);
  
/*!
 * Store the period time for task vt_gcs_gui_polling_gcs
 */
  __po_hi_milliseconds (&(period), 40);
  
/*!
 * \brief Making Periodic Task vt_gcs_gui_polling_gcs
 *
 * Make a periodic task according to AADL model requirements. The first 
 * parameter is the task identifier defined in deployment.h 
 * (x86_partition_vt_gcs_gui_polling_gcs_k) the second is the period defined
 *  in the AADL model. Third is the task priority ( 1), fourth is the stack 
 * size ( 50000 bytes) and last is the subprogram executed by the task
 */
  __po_hi_create_periodic_task (x86_partition_vt_gcs_gui_polling_gcs_k, &(period), 1, 50000, 0, vt_gcs_gui_polling_gcs_job);
