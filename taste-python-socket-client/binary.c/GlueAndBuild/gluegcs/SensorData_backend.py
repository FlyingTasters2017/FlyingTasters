...
def setMsgQ():
    global tmId
    tmId = PythonController.i_SensorData


def setUDP():
    global tmId
    from _InterfaceEnum import i_gcs_PI_SensorData
    tmId = i_gcs_PI_SensorData


def SensorData(tm_ptr, size):
...
def expect(Q, VNvalue, ignoreOther=False, timeout=None):
    ...
    while True:
        ...
        if msgId == tmId:
            #expectedValue = '{ SensorData ' + VNvalue + ' }'
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received SensorData with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected SensorData (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print 'Received other (%s), but still waiting for SensorData' % str(msgId)
            Q.task_done()
