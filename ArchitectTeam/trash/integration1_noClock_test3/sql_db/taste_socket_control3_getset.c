...
#include <assert.h>
#include <string.h>
#include "taste_socket_control3_getset.h"
...
{
    memcpy(dest, src, sizeof(MySensorData));
}

byte* MovePtrBySizeOf_MySensorData(byte *pData)
{
    return pData + sizeof(MySensorData);
}

byte* CreateInstanceOf_MySensorData() {
    MySensorData *p = (MySensorData*)malloc(sizeof(MySensorData));
    MySensorData_Initialize(p);
    return (byte*)p;
}
...
{
    memcpy(dest, src, sizeof(MyDroneData));
}

byte* MovePtrBySizeOf_MyDroneData(byte *pData)
{
    return pData + sizeof(MyDroneData);
}

byte* CreateInstanceOf_MyDroneData() {
    MyDroneData *p = (MyDroneData*)malloc(sizeof(MyDroneData));
    MyDroneData_Initialize(p);
    return (byte*)p;
}
