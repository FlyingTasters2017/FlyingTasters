...
#  SQLAlchemy models for types used in "dummy_integration.asn"
...
from dummy_integration_asn import (
    Agent, Area, Color, DroneControllerInput, DroneSensorData, FeedbackToGUI, HoverAction, MyBool, MyChoice, MyEnum, MyInteger, MyOctStr, MyReal, MySeq, MySeqOf, Orientation, Position, PositionSystemData, ReferenceFormation, ReferencePath, SafetyEvent, SystemState, T_Boolean, T_Int32, T_Int8, T_UInt32, T_UInt8, Trajectory, UserCommand, Velocity, WorldData
)
...
class WorldData_SQL(Base):
    ...
    def assignToASN1object(self, pyObj):
        state = pyObj.GetState()
        pyObj.Reset(state)
        self.rawASD.assignToASN1object(pyObj.rawASD)
        pyObj.Reset(state)
    ...
    fk_rawASD_iid = Column(Integer, ForeignKey('DroneSensorData.iid'), nullable=False)
    rawASD = relationship(
        "DroneSensorData_SQL",
        foreign_keys=[fk_rawASD_iid])

    def __init__(self, pyObj):
        state = pyObj.GetState()
        self.rawASD = DroneSensorData_SQL(pyObj.rawASD)
        pyObj.Reset(state)
