...
#include <assert.h>
#include <string.h>
#include "dummy_integration1_getset.h"
...
{
    memcpy(dest, src, sizeof(PixyData));
}

byte* MovePtrBySizeOf_PixyData(byte *pData)
{
    return pData + sizeof(PixyData);
}

byte* CreateInstanceOf_PixyData() {
    PixyData *p = (PixyData*)malloc(sizeof(PixyData));
    PixyData_Initialize(p);
    return (byte*)p;
}
...
{
    memcpy(dest, src, sizeof(Acceleration));
}

byte* MovePtrBySizeOf_Acceleration(byte *pData)
{
    return pData + sizeof(Acceleration);
}

byte* CreateInstanceOf_Acceleration() {
    Acceleration *p = (Acceleration*)malloc(sizeof(Acceleration));
    Acceleration_Initialize(p);
    return (byte*)p;
}
