    ...
    procedure runTransition(Id: Integer) is
        ...
        begin
            while (trId /= -1) loop
                case trId is
                    when 0 =>
                        -- NEXT_STATE Wait (13,18) at 303, 60
                        trId := -1;
                        ctxt.state := Wait;
                    ...
                    when 1 =>
                        ...
                        if (ctxt.user_input) /= 1 then
                            -- enable_pixycam(user_input) (23,25)
                            RIÜenable_pixycam(ctxt.user_input'Access);
                            -- NEXT_STATE wait (25,30) at 592, 329
                            trId := -1;
                            ctxt.state := wait;
                            goto next_transition;
                            -- ANSWER 1 (27,17)
                        elsif (ctxt.user_input) = 1 then
                            -- NEXT_STATE running (29,30) at 733, 279
                            trId := -1;
                            ctxt.state := running;
                            goto next_transition;
                        end if;
                    when 2 =>
                        ...
                        -- NEXT_STATE running (38,22) at 927, 271
                        trId := -1;
                        ctxt.state := running;
                        goto next_transition;
                    when 3 =>
                        ...
                        -- ANSWER /=1 (47,17)
                        if (ctxt.user_input) /= 1 then
                            -- NEXT_STATE wait (49,30) at 168, 283
                            trId := -1;
                            ctxt.state := wait;
                            goto next_transition;
                            -- ANSWER 1 (51,17)
                        elsif (ctxt.user_input) = 1 then
                            -- writeln('ENTER pixycam enable function') (53,25)
                            Put("ENTER pixycam enable function");
                            New_Line;
                            -- enable_pixycam(user_input) (55,27)
                            RIÜenable_pixycam(ctxt.user_input'Access);
                            -- writeln('EXIT pixycam enable function') (57,25)
                            Put("EXIT pixycam enable function");
                            New_Line;
                            -- NEXT_STATE running (59,30) at 350, 438
                            trId := -1;
                            ctxt.state := running;
                            goto next_transition;
                        end if;
