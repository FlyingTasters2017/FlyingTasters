...
i_GetHeight = ctypes.c_int.in_dll(PythonAccess, "ii_GetHeight").value
...
def Invoke_Takeoff(var_MyTrajectory):
    if -1 == SendTC_Takeoff(var_MyTrajectory._ptr):
        print 'Failed to send TC: Takeoff...\n'
        raise IOError("Takeoff")

def ProcessTM(self):
    if self.messageReceivedType == i_GetHeight:
        print "\n"+chr(27)+"[32m" + "Received Telemetry: GetHeight" + chr(27) + "[0m\n"
        backup = self._pMem
        # Read the data for param Height
        var_Height = dataview_uniq_asn.MyReal()
        var_Height.SetData(self._pMem)
