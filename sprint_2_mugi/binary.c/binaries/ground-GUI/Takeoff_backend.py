...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_Takeoff
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_ground_RI_Takeoff
    tcId = i_ground_RI_Takeoff
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global Takeoff_via_shared_lib
    shared_lib = True
    Takeoff_via_shared_lib = dll.ground_Takeoff
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.MyTrajectory_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def decode_ACN(ACN_encodedBuffer):
    ...
    decoded_value = ASN1.MyTrajectory()
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.MyTrajectory_REQUIRED_BYTES_FOR_ENCODING)
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_Takeoff(tc)
            except IOError:
                raise
    elif udp:
