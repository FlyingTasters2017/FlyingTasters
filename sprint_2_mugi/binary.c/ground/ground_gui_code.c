...
{
   //Create a variable of type T_GetHeight__data to be contained in the queue
   T_GetHeight__data GetHeight__data;

   GetHeight__data.Height = *Height;

   write_message_to_queue(ground_PI_queue_id, sizeof(T_GetHeight__data), (void*)&GetHeight__data, i_GetHeight);

   write_message_to_queue(ground_PI_Python_queue_id, sizeof(T_GetHeight__data), (void*)&GetHeight__data, i_GetHeight);
...
int compute_max_queue_size_element_for_PI(void)
{
   ...
   if( sizeof(T_GetHeight_message) > MAX_SIZE )
      MAX_SIZE = sizeof(T_GetHeight_message);
...
void ground_startup(void)
{
   ...
   fscanf(f, "%d", &msgsize_max);
    ...
    if (sizeof(T_GetHeight_message) > msgsize_max) {
        printf("[ERROR] The GUI is passing a message (GetHeight) which parameter size (%d) exceeds your system limit (%d).\nYou can extend this limit by running: \n    echo NUMBER | sudo tee /proc/sys/fs/mqueue/msgsize_max  # ...with NUMBER > %d\nYou can also make it permanent (check TASTE wiki)\n\n", sizeof(T_GetHeight_message), msgsize_max, sizeof(T_GetHeight_message));
        exit(1);
    }
