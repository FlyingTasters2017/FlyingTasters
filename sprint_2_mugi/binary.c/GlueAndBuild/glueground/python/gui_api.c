...
int SendTC_Takeoff(void *p_Trajectory)
{
    ...
    data.Trajectory = * (MyTrajectory *) p_Trajectory;
    if (((mqd_t)-1) != q) {
        write_message_to_queue(q, sizeof(Takeoff_TCDATA)-4, &data.Trajectory, data.tc_id);
    } else {
        return -1;
    }
    return 0;
