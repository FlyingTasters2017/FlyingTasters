...
def setMsgQ():
    global tmId
    tmId = PythonController.i_GetHeight


def setUDP():
    global tmId
    from _InterfaceEnum import i_ground_PI_GetHeight
    tmId = i_ground_PI_GetHeight


def GetHeight(tm_ptr, size):
...
def expect(Q, VNvalue, ignoreOther=False, timeout=None):
    ...
    while True:
        ...
        if msgId == tmId:
            #expectedValue = '{ GetHeight ' + VNvalue + ' }'
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received GetHeight with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected GetHeight (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print 'Received other (%s), but still waiting for GetHeight' % str(msgId)
            Q.task_done()
