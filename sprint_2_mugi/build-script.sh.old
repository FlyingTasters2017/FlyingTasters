...
# Generate code for OpenGEODE function obsw
cd "$SKELS"/obsw && opengeode --toAda obsw.pr system_structure.pr && cd $OLDPWD

cd "$SKELS" && rm -f obsw.zip && zip obsw obsw/* && cd $OLDPWD

cd "$SKELS" && rm -f controller.zip && zip controller controller/* && cd $OLDPWD

cd "$SKELS" && rm -f drone_model.zip && zip drone_model drone_model/* && cd $OLDPWD
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	--subSIMULINK drone_model:"$SKELS"/drone_model.zip \
