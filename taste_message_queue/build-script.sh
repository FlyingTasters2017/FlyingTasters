...
# Generate code for OpenGEODE function mainsupervisor
cd "$SKELS"/mainsupervisor && opengeode --toAda mainsupervisor.pr system_structure.pr && cd $OLDPWD

cd "$SKELS" && rm -f function1.zip && zip function1 function1/* && cd $OLDPWD

cd "$SKELS" && rm -f mainsupervisor.zip && zip mainsupervisor mainsupervisor/* && cd $OLDPWD
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	--subAda mainsupervisor:"$SKELS"/mainsupervisor.zip \
