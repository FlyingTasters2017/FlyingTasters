...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_TM_from_CF
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_function2_RI_TM_from_CF
    tcId = i_function2_RI_TM_from_CF
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global TM_from_CF_via_shared_lib
    shared_lib = True
    TM_from_CF_via_shared_lib = dll.function2_TM_from_CF
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.TM_T_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.TM_T_REQUIRED_BYTES_FOR_ENCODING)
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_TM_from_CF(tc)
            except IOError:
                raise
    elif udp:
